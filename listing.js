...
method.getListing = async function (searchKeyWord) {
  ...
  if (response.status == 200) {
    ...
    var history = new History();
    for (let i = 0; i < results.results.length; i++) {
      calls.push(john.getListingImages(results.results[i].listing_id).then(response => console.log(response.results)));
    }
    await Promise.all(calls).then(console.log(images));
